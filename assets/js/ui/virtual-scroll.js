/**
 * Virtual Scrolling UI Component
 * A performance-optimized scrolling system for large message lists
 */

/**
 * VirtualScroll class for efficiently rendering large lists
 */
export class VirtualScroll {
    /**
     * Create a new VirtualScroll instance
     * @param {Object} options - Configuration options
     * @param {string} options.container - Selector for container element
     * @param {string} options.messagesList - Selector for messages list element
     * @param {number} options.itemHeight - Average height of each item in pixels
     * @param {number} options.buffer - Number of extra items to render outside viewport
     * @param {number} options.threshold - Pixels before loading more content
     * @param {number} options.pageSize - Number of items to load per page
     * @param {string} options.groupId - Group ID for filtering messages
     */
    constructor(options = {}) {
    ...
    showMessageContextMenu(e, message) {
        ...
        const customEvent = new CustomEvent('message:contextmenu', {
            detail: {
                message,
                x: e.clientX,
                y: e.clientY
    ...
    startReply(message) {
        // Emit an event for the application to handle
        const customEvent = new CustomEvent('message:reply', {
            detail: { message }
        });
        document.dispatchEvent(customEvent);
    }
    ...
    toggleReaction(messageId, emoji) {
        // Emit an event for the application to handle
        const customEvent = new CustomEvent('message:reaction', {
     ...
     * @returns {boolean} Whether to auto-scroll
     */
    shouldAutoScroll() {
    ...
    showError(message) {
        ...
        // Emit an event for the application to handle
        const customEvent = new CustomEvent('virtualscroll:error', {
            detail: { message }
        });
        document.dispatchEvent(customEvent);
    }
...
/**
 * Create a virtual scroll for a chat container
 * @param {string} containerId - Container element ID
 * @param {Object} options - Additional options
 * @returns {VirtualScroll} Virtual scroll instance
 */
export function createChatVirtualScroll(containerId, options = {}) {
    return new VirtualScroll({
        container: `#${containerId}`,
        messagesList: `#${containerId} .messages-list`,
        itemHeight: 80,
        buffer: 5,
        threshold: 200,
        pageSize: 50,
