...
class EmojiPicker {
  ...
  render() {
    ...
    this.categories.forEach((category) => {
      html += `<div class="emoji-category-tab" data-category="${category.name}">${category.icon}</div>`;
    });
    ...
    this.categories.forEach((category) => {
      const emojis = this.emojis[category.name] || [];
      
      html += `
        <div class="emoji-category" data-category="${category.name}">
    ...
    this.showCategory(this.categories[0].name);
  }
  ...
  showCategory(categoryName) {
    ...
    const selectedCategory = this.container.querySelector(`.emoji-category[data-category="${categoryName}"]`);
    if (selectedCategory) {
      selectedCategory.style.display = 'block';
    }
    
    // Activate selected tab
    const selectedTab = this.container.querySelector(`.emoji-category-tab[data-category="${categoryName}"]`);
    if (selectedTab) {
  ...
  searchEmojis(query) {
    if (!query) {
      // Show first category if no query
      this.showCategory(this.categories[0].name);
      return;
    }
    ...
    // Create search results category
    let searchCategory = this.container.querySelector('.emoji-category[data-category="Search"]');
    
    if (!searchCategory) {
      searchCategory = document.createElement('div');
      searchCategory.className = 'emoji-category';
      searchCategory.setAttribute('data-category', 'Search');
      ...
      searchCategory.appendChild(grid);
      
      this.container.querySelector('.emoji-container').appendChild(searchCategory);
    }
    
    // Find matching emojis
    const results = this.searchEmojisByQuery(query);
    
    // Update grid
    const grid = searchCategory.querySelector('.emoji-grid');
    grid.innerHTML = '';
    
    if (results.length === 0) {
      grid.innerHTML = '<div class="emoji-no-results">No results found</div>';
    } else {
  ...
  searchEmojisByQuery(query) {
    ...
    // In a real implementation, each emoji would have keywords/tags
    allEmojis.forEach((emoji) => {
      if (results.includes(emoji)) return;
      
      // Add if the character includes the query
  ...
  selectEmoji(emoji) {
    ...
    // Emit event
    eventBus.emit('emoji:selected', emoji);
    
    // Insert into input if defined
    if (this.options && this.options.inputElement) {
      const input = this.options.inputElement;
  ...
  addToRecentEmojis(emoji) {
    ...
    // Update recent emojis in storage
  ...
  show(options = {}) {
    ...
    // Position picker
    if (options.position) {
      this.container.style.top = `${options.position.top}px`;
      this.container.style.left = `${options.position.left}px`;
    }
