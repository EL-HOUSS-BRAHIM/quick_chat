...
export class Card {
    ...
    ensureAccessibility() {
        ...
        if (this.options.interactive) {
            ...
            // Add accessible name if not provided
            if (!this.element.getAttribute('aria-label') && !this.element.getAttribute('aria-labelledby')) {
                const titleText = this.options.title || 'Interactive card';
                this.element.setAttribute('aria-label', titleText);
            }
        }
        ...
        images.forEach(img => {
            if (!img.getAttribute('alt')) {
                img.setAttribute('alt', img.getAttribute('data-alt') || 'Card image');
            }
        });
    ...
    setLoading(loading) {
        ...
        if (loading && !overlay) {
            this.element.insertAdjacentHTML('afterbegin', 
                '<div class="card__loading-overlay" aria-hidden="true"><div class="loading-spinner"></div></div>'
            );
    ...
    update() {
        ...
        // Reapply accessibility features after update
        this.ensureAccessibility();
    }
